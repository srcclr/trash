...
func TestListPackages(t *testing.T) {
	assert := require.New(t)
	p := listPackages("github.com/srcclr/trash", "vendor")
	logrus.Debug(p)
	assert.Equal(3, len(p))
	assert.Contains(p, "github.com/srcclr/trash")
	assert.Contains(p, "github.com/srcclr/trash/util")
	assert.Contains(p, "github.com/srcclr/trash/conf")
}
